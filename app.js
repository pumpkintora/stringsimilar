...
const acraResourceIds = {
    ...
    others: 'e8732ea3-adca-4ca0-9fef-2b90c7a24f06',
}

const config = {
    method: 'get',
    url: `https://data.gov.sg/api/action/datastore_search?resource_id=${acraResourceIds['a']}&q=amo`,
}

axios(config).then((res) => {
    res.data.result.records.forEach((r) => {
        if (r.entity_name.includes('AMO GASTRO & WINE BAR')) console.log(r)
    })
})

console.log(stringSimilarity.compareTwoStrings('SHOPEE SINGAPORE', 'SHOPEE'))
